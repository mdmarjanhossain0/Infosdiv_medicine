...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with open("drug_generics_bn.json") as json_file:
            ...
            for item in data:
                ...
                if (
                    item.get("medicine_generic_id", None) == None
                    or item.get("medicine_generic_id", None) == ""
                ):
                    medicine_generic_id = None
                else:
                    medicine_generic_id = item.get("medicine_generic_id", None)
                medicine = GenericMedicineBangla(
                    pk=item.get("bn_generic_id"),
                    medicine_generic_en_id=medicine_generic_id,
